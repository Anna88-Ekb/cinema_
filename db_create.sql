...
CREATE TABLE
  ...
  , cinema_duration SMALLINT NOT NULL
...
CREATE TABLE
  ...
  , cinema_session_session_id INTEGER NULL,
    transact_transact_id  BIGINT NOT NULL
  );


CREATE TABLE transact (
    transact_id        BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    transact_sum       NUMERIC(10, 2) NOT NULL,
...
ALTER TABLE ticket
    ADD CONSTRAINT ticket_transact_fk FOREIGN KEY ( transact_transact_id )
        REFERENCES transact ( transact_id );

ALTER TABLE transact
    ADD CONSTRAINT transact_client_fk FOREIGN KEY ( client_client_id )
        REFERENCES client ( client_id );

ALTER TABLE transact
    ADD CONSTRAINT transact_unreg_user_fk FOREIGN KEY ( unreg_user_user_id )
        REFERENCES unreg_user ( user_id );

ALTER TABLE transact
    ADD CONSTRAINT transact_worker_fk FOREIGN KEY ( worker_worker_id )
